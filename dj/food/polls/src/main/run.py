...
#body_text = """
#<xml>
#<ToUserName><![CDATA[odIgavz5qd3QrLSHyK5nefTgyIH4]]></ToUserName>
#<FromUserName><![CDATA[odIgavz5qd3QrLSHyK5nefTgyIH4]]></FromUserName>
...
#</xml>
#"""
#signature = wechat.generate_jsapi_signature(timestamp, nonce, 'http://wechat.grabbieteam.com')
## 实例化 wechat
## wechat = WechatBasic(token=token)
## 对签名进行校验
#
## token = wechat.get_access_token().access_token
## if wechat.check_signature(signature=signature, timestamp=timestamp, nonce=nonce):
## 对 XML 数据进行解析 (必要, 否则不可执行 response_text, response_image 等操作)
#wechat.response_text("hello there!")
#wechat.parse_data(body_text)
## 获得解析结果, message 为 WechatMessage 对象 (wechat_sdk.messages中定义)
#message = wechat.get_message()
#
#if isinstance(wechat.message, TextMessage):
#    print wechat.message.content
#response = None
...
#print response
#pprint.pprint(wechat.message.content)
## else:
##     print "error"
